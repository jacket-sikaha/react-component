import { ControlPosition } from '@/types/enum';
import type { GeolocationResult, TDTGeolocation, TDTMap } from '@/types/TDT';
...
// 自定义控件采用react实现方式
export default function CustomPositioningControl(props: { map: TDTMap | undefined }) {
  const { map } = props;
  const [showBasic, setShowBasic] = useState(false);
  const locationSearching = useRef<HTMLDivElement>(null);
  //创建定位对象
  const getCurrentPositionCallback = function (this: TDTGeolocation, e: GeolocationResult | null) {
  ...
  useEffect(() => {
    map?.addEventListener('addcontrol', () => {
      // 控件加载完成再显示
      setShowBasic(true);
    });
  ...
  return (
    <div
      className={`mx-3 my-2 max-w-fit cursor-pointer rounded-lg border bg-white p-1 ${
